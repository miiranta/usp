...
class GELU3(nn.Module):
    """Per-neuron sign-similarity habituation GELU.

    Maintains a single EMA prototype vector (D,). For each activation
    dimension k the suppression is driven by sign agreement between the
    current activation and the stored prototype component:
    ...
    After the scaled forward pass the prototype is updated:
        prototype = d * prototype + (1 − d) * mean(x over B, T)

    Learnable parameters:
        log_alpha[D] → alpha[k] ∈ (0,1) : per-neuron suppression strength
        logit_decay  → d       ∈ (0,1) : EMA decay (scalar)
    """
    ...
    def __init__(self, ema_decay: float = None):
        ...
        self._ready        = False
        self.log_alpha: nn.Parameter = None   # (D,), registered lazily on first forward
        self.logit_decay   = nn.Parameter(
            torch.tensor(math.log(init_decay / (1.0 - init_decay)))
        )
    ...
    def _init_alpha(self, D: int, device: torch.device, dtype: torch.dtype):
        """Lazily create per-neuron alpha when D is first known."""
        if self.log_alpha is None:
            init_logit = math.log(Config.GELU2_BLEND / (1.0 - Config.GELU2_BLEND))
            self.log_alpha = nn.Parameter(
                torch.full((D,), init_logit, device=device, dtype=dtype)
            )

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (B, T, D)
        D = x.shape[-1]
        self._init_alpha(D, x.device, x.dtype)

        d_val = torch.sigmoid(self.logit_decay).detach().item()   # plain float for EMA
