\section{Results}
    ...
    \begin{table*}[t]
        \centering
        \caption{Statistical significance of RMSE improvements across all experimental runs.}
        \label{tab:significance_results}
        \begin{tabular}{llrrrrrr}
            \hline
            \textbf{Model} & \textbf{Correction} & \textbf{Mean (\%)} & \textbf{Std (\%)} & \textbf{n} & \textbf{t-statistic} & \textbf{p-value} & \textbf{Significant} \\
            \hline
    ...
    Table~\ref{tab:significance_results} presents the statistical significance analysis of RMSE improvements. All configurations demonstrate highly significant improvements ($p < 0.001$), confirming that the observed performance gains are not due to random variation. The ARIMA models show particularly strong improvements, with mean RMSE reductions of 1.2209\% (uncorrected) and 0.7403\% (corrected). LSTM models exhibit more modest but still significant improvements of 0.1581\% (uncorrected) and 0.2534\% (corrected). The large sample sizes ($n = 6{,}132$) and substantial t-statistics provide robust evidence for the effectiveness of incorporating sentiment analysis into inflation forecasting models.
