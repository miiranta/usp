...
\subsection{Creating the Phrase Dataset}
    ...
    \subsubsection{Scraping}
        ...
        We collected COPOM (Central Bank of Brazil's Monetary Policy Committee) minutes using Python and Selenium. We accessed the official COPOM minutes listing~\cite{BCB_COPOM_Minutes}, which contains all available minutes. For each minute, we downloaded both the HTML and PDF content when available.
    ...
    \subsubsection{Parsing}
        ...
        \begin{enumerate}
            ...
            \item{Type-Specific Pre-Processing}
                ...
                PDF file: if it exists, we used SpaCyLayout~\cite{neumann2019scispacy} with the \texttt{pt\_core\_news\_lg} model to extract individual phrases from PDF documents.
    ...
    \subsubsection{Phrase Selection}
        ...
        We performed \textbf{dense passage retrieval}~\cite{karpukhin2020dense} using semantic similarity filtering. We computed dense vector representations (embeddings) for all phrases using the \textbf{Qwen3-Embedding-0.6B}~\cite{zhang2025qwen3embedding} model and computed the cosine similarity between each phrase embedding and the embedding of the target concept ``inflation''. We retained only phrases with a cosine similarity score exceeding a threshold of 0.6, thereby selecting phrases semantically related to inflation concepts.

        The implementation utilized PyTorch~\cite{paszke2019pytorch} for GPU acceleration, pandas for data manipulation, scikit-learn for similarity computations, and the LangChain HuggingFace integration for embedding generation.
...
\subsection{Human Evaluation Dataset}
    ...
    \begin{enumerate}
        ...
        We requested collaborating universities (USP and Unicamp) to share the website with their economics-related graduate students. The evaluation form is publicly accessible~\cite{Inflation_Form}.
...
\subsection{Testing Inflation Prediction Performance}
    ...
    \subsubsection{Creating the Input Datasets}
        ...
        Finally, for each tuple (\(U_{i}\), \(V_{j}\), \(eq_{k}\)) created, we will have 3 new associated lists of tuples in the form of (\({Inflation}, {Sentiment}\)), each will be called \(IN_{ijkm}\) where \(i\) is the LLM model combination used; \(j\) is the human evaluation dataset used for bias correction; \(k\) is the equation type used for bias correction; and \(m\) \(\in\) \{\textit{Baseline}, \textit{Without Correction}, or \textit{With Correction}.\}
    ...
    \subsubsection{Running the Tests}
        ...
        \textbf{For each} \(IN_{ijkm}\) in \(IN\), we will run both ARIMA and LSTM~\cite{hochreiter1997lstm} inflation prediction models using the respective dataset as input. Data was split 70/30 for training and testing.

        We employed ARIMA with sentiment as exogenous variable~\cite{moslemi2024comprehensive} using walk-forward validation, and LSTM with 5000 neurons trained with NAdam optimizer~\cite{dozat2016nadam} (learning rate 0.001, max 10,000 epochs, early stopping patience 10). The choice of a highly parameterized LSTM architecture aligns with recent insights on double descent~\cite{schaeffer2023double}, where increasing model complexity can lead to improved generalization in the overparameterized regime. Both models were evaluated using Root Mean Squared Error (RMSE).
