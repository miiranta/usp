...
pj = np.full((b), 1)  # Por que definir 2 p?
pij = np.full((b), 1)
...
W = np.full((s, b), 1) #GOV: Precisa ser tudo 1!
...
def nextP(P, R, W, pTil, rTil, wTil, qTil, cTil):
    ...
    for j in range(b):
        ...
        #GOV
        #Não sei se é necessário conferir se wBar[j] == W[:, j], já que se o W é sempre 1, o WBar é sempre 1, nesse caso
        #Mesmo questionamento é válido para outras partes
...
p = calcPontoFixo(pj, R, W, pTil, rTil, wTil, qTil, cTil) #GOV - faz o ponto fixo com o pj
print("Peq\n", p)
...
newQ = calcularSistemaLinear(K, pj, R, A, W, Af, Ak, Ah, Az, ac, ak, az, ah, al, lamh, alpha, csi, delta, v, w, theta, ro, nEst, oEst, pTil, qTil, rTil, wTil, cTil, wCirc, pCirc, wBar, hBar, gammal, gammac, gamma, gammah, phic, phil)

#GOV - Sistema linear (incompelto)
print("\nQeq\n", newQ)

#GOV - print do p e do P
print(p, P)
...
for x in x_gamma:   
    newQ = calcularSistemaLinear(K, pj, R, A, W, Af, Ak, Ah, Az, ac, ak, az, ah, al, lamh, alpha, csi, delta, v, w, theta, ro, nEst, oEst, pTil, qTil, rTil, wTil, cTil, wCirc, pCirc, wBar, hBar, gammal, gammac, x, gammah, phic, phil)
