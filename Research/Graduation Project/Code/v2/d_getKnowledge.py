...
# Change filter 0 to infinity (no filter)
appended_benchmarks_df['filter'] = appended_benchmarks_df['filter'].replace(0, 5)
...
# Define custom functions to test
FUNCTIONS_TO_TEST = {
    'linear': {
...
plt.savefig(os.path.join(OUTPUT_FOLDER, 'filter_vs_complexity.png'))
plt.close()
