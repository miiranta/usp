...
\section{Filter dimension}
    ...
    The regression for average bins exhibits a weak fit ($R^2 = 0.045$), indicative of high variability. Conversely, the maximum bins demonstrate an exceptional fit ($R^2 = 0.999$). This disparity is expected, as the maximum bins metric consists of a single data point per filtering setting, which simplifies curve fitting. A more equitable comparison of $R^2$ values would be achieved by performing regression on the mean value for each filtering setting rather than the entire dataset. Such an approach yields an $R^2$ of 0.999, while retaining the same function parameters observed in the regression over the full dataset.
    
    The maximum bins (green dashed line) grows a little bit faster than the average bins (red line) as the filtering becomes less aggressive. Mathematically, while maximum bins grow at a rate of $e ^{0.250x}$, average bins only grow $e ^{0.236x}$. The fact that both curves follow an \textbf{exponential trend}, however, is noteworthy.
    ...
    As anticipated, both the average (red line) and maximum (green dashed line) complexity values exhibit a \textbf{logarithmic trend} ($R^2 = 0.014$ and $R^2 = 0.578$, respectively). The maximum fit demonstrates a significantly higher $R^2$ value compared to the average fit, for the same reasons detailed in Figure \ref{fig:regression_bins_per_filtering}. Applying the previously described adjustment performing regression on the mean values per filtering setting results in an $R^2$ of 0.977, while preserving the function parameters derived from the full dataset regression.
    
    The maximum fit in this instance is notably inferior to that observed in Figure \ref{fig:regression_bins_per_filtering}, likely attributable to the anomalous spike at 0.125 $\sigma$. Both curves show a downward trend as the filtering becomes more aggressive that accelerates the more we approach 0 $\sigma$, where all the values are removed. This acceleration is faster in the maximum complexity fit, as expected.
...
\label{sec:complexity_vs_num_params_results}
    ...
    \begin{figure}[H] 
        \centering
        \includegraphics[width=1.0\linewidth, height=0.45\textheight, keepaspectratio]{img/noembed-weights/param_average_complexity_histogram.png}
...
\section{Complexity vs Number of bins}
    ...
    \begin{figure}[H] 
        \centering
        \includegraphics[width=1.0\linewidth, height=0.45\textheight, keepaspectratio]{img/noembed-weights/bin_average_complexity_histogram.png}
...
\label{sec:complexity_vs_benchmark_performance_results}
    ...
    We also found the statistical significance of the correlations found using t-tests \cite{Fisher1921} shown in table \ref{tab:statistical_significance}.
    ...
    \begin{figure}[H] 
        \centering
        \includegraphics[width=1.0\linewidth, height=0.35\textheight, keepaspectratio]{img/noembed-weights/comp_lmarena_regression.png}
    ...
    \begin{figure}[H] 
        \centering
        \includegraphics[width=1.0\linewidth, height=0.35\textheight, keepaspectratio]{img/noembed-weights/comp_mmlu_regression.png}
    ...
    \begin{figure}[H] 
        \centering
        \includegraphics[width=1.0\linewidth, height=0.35\textheight, keepaspectratio]{img/noembed-weights/comp_mmlupro_regression.png}
    ...
    \begin{figure}[H] 
        \centering
        \includegraphics[width=1.0\linewidth, height=0.35\textheight, keepaspectratio]{img/noembed-weights/comp_openllm_regression.png}
    ...
    \begin{figure}[H] 
        \centering
        \includegraphics[width=1.0\linewidth, height=0.35\textheight, keepaspectratio]{img/noembed-weights/comp_all_regression.png}
    ...
    As a bonus, for the sake of completeness, we made a bar plot ranking the top 20 configurations (model + weight-type combination) sorted by highest correlation between LMC complexity and benchmark performance (all benchmarks aggregated) shown in figure \ref{fig:top_20_correlations}.
