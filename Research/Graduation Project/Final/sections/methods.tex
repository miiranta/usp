...
\subsection{Comparing LMC Complexity and Inference Capability}
    ...
    \label{sec:regression_procedures}
        ...
        We will also using two important concepts defined as follows:
        ...
        \begin{itemize}
            ...
            For comparisions, we will use the \textbf{Pearson correlation} \cite{Freedman2007} (linear correlation coeficient). This can be computed as:
    ...
    \subsubsection{Best filtering setting}
    \label{sec:best_filtering_setting}
        ...
        The filtering dimension will be used to determine the best general filtering setting following the criteria defined in section \ref{sec:filtering}. After defining the best filtering setting, we will discart all the runs where \( \sigma_{\text{filter}} \) is different from the chosen one, creating a new set $\mathbf{R}_{\mathbf{best}}$. There will be a total of: \( 35 \text{ models } \times 15 \text{ weight-type combinations } \times 1 \text{ filtering setting } = 525 \text{ datapoints} \).

        We will also make a free regression for the data obtained in step 1 and 2 to analyze how filtering strength affects histogram bin count and complexity, respectively.
    ...
    \subsubsection{Influence of weight types}
        ...
        Since we are using combinations simetrically (each weight type appears the same amount of times in \( R_{best} \)), it is not necessary to select a best weight-type combination, like it is done in \ref{sec:best_filtering_setting}, because it won't add any bias to the analysis.
