...
In this work, we investigated the relationship between the LMC statistical complexity of neural network weights and their generalization performance across various benchmarks. Our analysis revealed that, even with a statistically significant result, \textbf{it is not yet possible to say there is a general correlation between LMC complexity and benchmark performance}. 
...
Until the reasons why the opposite facts happen is clarified, it is hard to not consider that the results might be outcomes of outliers. The fact that these exist might indicate that LMC Complexity is not a reliable predictor of neural network generalization capability in the first place.

In order to validate or falsify the initial hypothesis, a new study with a new approach would be necessary. A new methodology independent of benchmark results should be better given the difficulty in obtaining sufficient data for statistical significance and the imperfect nature of their performance assessments. As already mentioned in section \ref{sec:generalization_capability}, a good option is to use test loss in a new model trained from scratch as a performance metric.
