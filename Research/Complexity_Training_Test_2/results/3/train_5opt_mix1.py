...
def save_results_to_csv(output_dir, results):
    ...
    with open(file_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Epoch', 'Train Loss', 'Val Loss', 'Metric Value'])
        for row in results:
            writer.writerow(row)
...
def main():
    ...
    for metric_name in METRICS_TO_RUN:
        ...
        output_dir = f"output_5opt_mix/{clean_name}_{direction}"
        ...
        results = []
        # output_dir = f"output_5opt_mix/{metric_name}_{direction}"
        ...
        for epoch in range(Config.EPOCHS):
            ...
            results.append([epoch+1, train_loss, val_loss, train_metric])
