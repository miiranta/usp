...
//------------------------------------------------------------
// pre-condicao: Arvore nao esta vazia e há pelo menos dois nós
// pos-condicao: Retorna o segundo menor valor encontrado na arvore binaria de busca
template< class TreeEntry >
...
{ 
  ...
  //t inicia como root
  //p será o pai de t
  TreePointer t = root, p = root;

  //Se a arvore estiver vazia, retorna o menor valor possível do tipo int
  if(t == NULL)
  { cout << "Arvore vazia!" << endl;
    return INT_MIN;
  }

  //Se a arvore tem apenas um nó, retorna o menor valor possível do tipo int
  if(t->leftNode == NULL && t->rightNode == NULL)
  { cout << "Arvore com apenas um no!" << endl;
    return INT_MIN;
  }

  //Avance t->leftNode até chegar no nó com valor mínimo (t->leftNode == NULL)
  while(t->leftNode != NULL)
  { p = t;
    t = t->leftNode;
  }

  //Se t->rightNode == NULL, p->entry é o segundo menor valor
  if(t->rightNode == NULL){
    return p->entry;
  
  //Se t->rightNode != NULL, o segundo menor valor é o valor mínimo da subárvore apontada por t->rightNode
  }else{
    t = t->rightNode;
